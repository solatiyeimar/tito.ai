...
async def run_bot(bot_class: Type, config: BotConfig, room_url: str, token: str) -> None:
    ...
    bot = None
    try:
        logger.info(f"Iniciando bot tipo {config.bot_type} en sala: {room_url}")
        ...
        # Start the bot.
        await bot.start()
        logger.info(f"Bot finalizado en sala: {room_url}")
        ...
        if bot:
            await bot.cleanup()
            logger.info(f"Recursos del bot limpiados para la sala: {room_url}")
...
def cli() -> None:
    ...
    parser.add_argument(
        ...
        "--google-temperature",
        type=float,
        help="Override GOOGLE_TEMPERATURE (default: 1.0)",
    )
    ...
    parser.add_argument(
        ...
        help="Override TTS_PROVIDER (default: elevenlabs)",
    )
    ...
    parser.add_argument(
        "--cartesia-voice",
        type=str,
        help="Override CARTESIA_VOICE (default: CO-M-1)",
    )
    parser.add_argument(
        "--elevenlabs-voice-id",
        type=str,
        help="Override ELEVENLABS_VOICE_ID (default: J4vZAFDEcpenkMp3f3R9)",
    )
    parser.add_argument(
        "--rime-voice-id",
        type=str,
