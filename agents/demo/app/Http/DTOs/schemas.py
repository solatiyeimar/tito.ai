...
class STTConfig(BaseModel):
    provider: Literal["deepgram", "gladia", "assemblyai", "groq"] = "deepgram"
    model: Optional[str] = None
    language: str = "en"
    params: Dict[str, Any] = Field(default_factory=dict)
...
class TTSConfig(BaseModel):
    provider: Literal["deepgram", "cartesia", "elevenlabs", "rime", "playht", "openai", "azure"] = "cartesia"
...
class WebhookConfig(BaseModel):
    ...
    events: List[Literal["call_started", "call_ended", "participant_joined", "participant_left", "transcription", "call_transfer"]] = Field(
        default_factory=lambda: ["call_started", "call_ended"]
    )

# --- Flow Configuration ---

class FlowFunctionSchema(BaseModel):
    name: str
    description: str
    properties: Dict[str, Any] = Field(default_factory=dict)
    required: List[str] = Field(default_factory=list)
    handler: str  # Name of the handler function to be resolved at runtime
    transition_callback: Optional[str] = None  # Name of the transition callback
...
class FlowNode(BaseModel):
    ...
    functions: List[FlowFunctionSchema] = Field(default_factory=list)
...
class CallResponse(BaseModel):
    ...
    def tts_provider(self) -> str:
        return self.io_layer.tts.provider
        
    @property
    def tts_voice(self) -> Optional[str]:
        return self.io_layer.tts.voice_id
        
    @property
    def stt_provider(self) -> str:
        return self.io_layer.stt.provider
